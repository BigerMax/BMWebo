...
class BMPublishView: UIView {
    ...
    private let btnAnimationDuration = 0.025
    ...
    private let buttonInfos = [["imageName": "tabbar_compose_idea", "title": "文字", "clsName": "BMWriteController"],
    ...
    func setupUI() {
        ...
        sloganView.snp.makeConstraints { (make) in
            ...
            make.top.equalTo(BMLayout.Layout(100))
            make.width.equalTo(BMLayout.Layout(154))
            make.height.equalTo(BMLayout.Layout(40))
        }
        ...
        bottomView.snp.makeConstraints { (make) in
            ...
            make.height.equalTo(BM_bottomTabBarHeight)
        }
        ...
        closeBtn.snp.makeConstraints { (make) in
            ...
            make.width.height.equalTo(BMLayout.Layout(25))
        }
        ...
        returnBtn.addTarget(self, action: #selector(clickReturenBtn), for: .touchUpInside)
        ...
        scrollView.snp.makeConstraints { (make) in
            ...
            make.height.equalTo(BMLayout.Layout(224))
            make.bottom.equalTo(closeBtn.snp.top).offset(-BMLayout.Layout(56))
        }
    ...
    @objc func clickButton(selectedButton:BMCompostTypeButton) {
    ...
    @objc func clickMoreButton() {
        ...
        returnBtn.snp.makeConstraints { (make) in
            make.centerX.equalToSuperview().offset(-BMLayout.Layout(margin))
        }
        closeBtn.snp.updateConstraints { (make) in
            make.centerX.equalToSuperview().offset(BMLayout.Layout(margin))
        }
 ...
 private extension BMPublishView {
    ...
    func setupCompostButtons(parentView: UIView) {
        ...
        for i in 0...maxPage{
            ...
            creatButtons(parentView: view, index: i * pageCount)
        }
    }
    
    func creatButtons(parentView: UIView, index: Int) {
        for i in index..<(index + pageCount){
            ...
            let button = BMCompostTypeButton(imageName: imageName, title: title)
        ...
        let lineNum:CGFloat = 3
        let margin = (UIScreen.bm_screenW - lineNum * buttonWH) / (lineNum + 1)
        
        for (index, btn) in parentView.subviews.enumerated() {
            let row = index / Int(lineNum)
            let colum = index % Int(lineNum)
            let x = margin + CGFloat(colum) * (buttonWH + margin)
 ...
 private extension BMPublishView{
    ...
    func showButtonAnimate () {
        let backfgroundView = scrollView.subviews[0]
        
        for (index, btn) in backfgroundView.subviews.enumerated() {
            ...
            anim.beginTime = CACurrentMediaTime() + CFTimeInterval(index) * btnAnimationDuration
    ...
    func hideButtons() {
        ...
        for (index,btn) in view.subviews.enumerated().reversed(){
            ...
            anim.beginTime = CACurrentMediaTime() + CFTimeInterval(hiddenIndexOrder) * btnAnimationDuration
            btn.pop_add(anim, forKey: nil)
