//
//  BMSQLiteManager.swift
//  BMWebo
//
//  Created by BiggerMax on 2020/5/25.
...
class BMSQLiteManager {
    ///数据库单例
    static let shared = BMSQLiteManager()
    ...
    private init(){
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(clearDBCache), name: UIApplication.willTerminateNotification, object: nil)
    ...
    @objc private func clearDBCache(){
        let dateString = Date.bm_dateString(delta: maxDBCacheTime)
...
extension BMSQLiteManager {
            ...
            while rs.next() {
                ...
                for column in 0..<columnCount {
                    ...
                    guard let name = rs.columnName(for: column),
                        let value = rs.object(forColumnIndex: column) else { continue }
    ...
    func createTable() {
        ...
        // 1. SQL
        guard let path = Bundle.main.path(forResource: "BMWeibo.sql", ofType: nil),
...
extension BMSQLiteManager{
    ...
    /// - Returns: 微博的字典的数组,将数据库中status字段对应的二进制数据反序列化，生成字典
    func loadWeiboStatus(userId: String, since_id: Int64 = 0, max_id: Int64 = 0) -> [[String: AnyObject]] {
        ...
        for dict in array {
            ...
            guard let jsonData = dict["status"] as? Data,
                let json = try? JSONSerialization.jsonObject(with: jsonData, options: []) as? [String: AnyObject] else { continue }
        ...
        queue.inTransaction { (db, rollback) in
            ...
            for dict in array {
                ...
                guard let statusId = dict["idstr"] as? String,
                    let jsonData = try? JSONSerialization.data(withJSONObject: dict, options: []) else { continue }
            
                //执行SQL(参数要根据sql的问号 -> 按顺序)
